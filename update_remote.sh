...
scp -i ${key_path} update_gpumon.txt ubuntu@${instance_ip}:/home/ubuntu/update_gpumon.sh
error=$?
if [ "${error}" != "0" ]; then
...
echo "We are done, instance:${instance_ip} updated with new version of gpumon.py - please wait 10 seconds"
sleep 10
